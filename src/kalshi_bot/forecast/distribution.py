...
class ForecastDistribution:
    ...
    # NBM published percentiles stored for reporting.
    ...
    # Prevent the CDF from claiming certainty for deep tail contracts.
    # NBM percentiles only cover p10–p90; anything beyond that extrapolates,
    # and the extrapolation hits 1.0 for contracts far outside the range.
    # Capping at 0.97 keeps extreme-tail contracts below our edge threshold.
    MAX_PROB: float = 0.97
    ...
    def cdf(self, x: float) -> float:
        ...
        if self.normal_weight > 0 and self.normal_mean is not None and self.normal_sd:
            ...
            raw = (1 - self.normal_weight) * c_pwl + self.normal_weight * c_norm
        else:
            raw = c_pwl
        return min(raw, self.MAX_PROB)
    ...
    def prob_above(self, x: float) -> float:
        return min(1.0 - self.cdf(x), self.MAX_PROB)
